...
> = (props) => {
  ...
  const [userPseudo, setUserPseudo] = useState("N/A")
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      }console.log("data retrieved HERE !", data);

      setUserPseudo(data.userName);
      setUserData(data);
    };
    if (userDonutId > 0) {
  ...
  const handleChangeUserName = async () => {
    ...
    let req = await instance.setDonutName(userDonutId, newUserName);
    await req.wait()
    const req2 = await instance.getDonutInfos(userDonutId);
    console.log("TEST HERE", req2);
    setUserPseudo(req2.userName);
    // setUserData(userData => {...userData, userName:  req2.userName});
    setChangeNameActive();
  };
  return (
    <>
