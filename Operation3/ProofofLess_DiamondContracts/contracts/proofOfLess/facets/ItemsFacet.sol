...
import {LibAppStorage, AppStorage, Modifiers, ItemType, DonutProfil, ItemType, WearableSet, NUMERIC_TRAITS_NUM, EQUIPPED_WEARABLE_SLOTS} from "../libraries/LibAppStorage.sol";
...
contract ItemsFacet is Modifiers {
   ...
   |__________________________________*/
    ...
    ///@notice Returns the balances for all ERC1155 items for a ERC721 token (donut)
    ...
    {
        itemBalancesOfTokenWithTypes_ = LibItems._itemBalancesOfTokenWithTypes(_tokenContract, _tokenId);
    }
    ...
    function equippedWearables(uint256 _tokenId) external view onlyDonutOwnerOrApproved(_tokenId) returns (uint16[EQUIPPED_WEARABLE_SLOTS] memory wearableIds_) {
        wearableIds_ = s.members[_tokenId].equippedWearables;
    }
    ...
    */
    function setBaseURI(string memory _value) external onlyCoreTeam {
   ...
   |__________________________________*/
    ...
    ///@notice Allow the donut owner to equip/unequip wearables
    ...
    ///@param _tokenId The identifier of the donut to make changes to
    ///@param _wearablesToEquip An array containing the identifiers of the wearables to equip
    function equipWearables(uint256 _tokenId, uint16[EQUIPPED_WEARABLE_SLOTS] calldata _wearablesToEquip) external onlyDonutOwnerOrApproved(_tokenId) {
        DonutProfil storage donut = s.members[_tokenId];
        require(donut.isActive, "ItemsFacet : Only valid for Active Donuts");
        emit EquipWearables(_tokenId, donut.equippedWearables, _wearablesToEquip);

        address sender = LibMeta.msgSender();

        for (uint256 slot; slot < EQUIPPED_WEARABLE_SLOTS; slot++) {
            uint256 toEquipId = _wearablesToEquip[slot];
            uint256 existingEquippedWearableId = donut.equippedWearables[slot];
            ...
            donut.equippedWearables[slot] = uint16(toEquipId);
            ...
            if (existingEquippedWearableId != 0 && s.itemTypes[existingEquippedWearableId].canBeTransferred) {
                // remove wearable from donut and transfer item to owner
                LibItems._removeFromParent(address(this), _tokenId, existingEquippedWearableId, 1);
                LibItems._addToOwner(sender, existingEquippedWearableId, 1);
            ...
            if (toEquipId != 0) {
                ...
                require(LibDonuts._donutLevel(donut.level) >= itemType.minLevel, "ItemsFacet: Donut level lower than minLevel");
                ...
                //         uint256 collateralIndex = s.collateralTypeIndexes[donuts.collateralType];
                ...
                //Then check if this wearable is in the donut inventory
                ...
                if (nftBalance < neededBalance) {
                    ...
                    //Transfer to Donut
                    LibItems._removeFromOwner(sender, toEquipId, balToTransfer);
                    LibItems._addToParent(address(this), _tokenId, toEquipId, balToTransfer);
        ...
        LibDonuts._interact(_tokenId);
    }

    ///@notice Allow the owner of an NFT to use multiple consumable items for his donut
    ///@dev Consumables can be used to boost kinship/XP of a donut
    ///@param _tokenId Identtifier of the donut to use the consumables on
    ...
    function useConsumables(
    ...
    ) external onlyDonutOwnerOrApproved(_tokenId) {
        require(_itemIds.length == _quantities.length, "ItemsFacet : _itemIds length != _quantities length");
        require(s.members[_tokenId].isActive == true, "ItemsFacet : Only valid for Active Donut");
        ...
        for (uint256 i; i < _itemIds.length; i++) {
            ...
            LibItems._removeFromOwner(sender, itemId, quantity);

            {
                ...
                if (boost) {
                    // s.members[_tokenId].lastTemporaryBoost = uint40(block.timestamp);
                    s.members[_tokenId].temporaryTraitBoosts = itemType.traitModifiers;
                }
            }

            //Increase experience
            if (itemType.experienceBonus > 0) {
                uint256 experience = (uint256(itemType.experienceBonus) * quantity) + s.members[_tokenId].experience;
                s.members[_tokenId].experience = experience;
            }
