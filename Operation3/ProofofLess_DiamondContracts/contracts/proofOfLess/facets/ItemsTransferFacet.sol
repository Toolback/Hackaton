...
import {Modifiers, EQUIPPED_WEARABLE_SLOTS, DonutProfil} from "../libraries/LibAppStorage.sol";
...
contract ItemsTransferFacet is Modifiers {
    ...
    */
    function safeTransferFrom(
        ...
        LibItems._removeFromOwner(_from, _id, _value);
        LibItems._addToOwner(_to, _id, _value);
    ...
    */
    function safeBatchTransferFrom(
        ...
        for (uint256 i; i < _ids.length; i++) {
            ...
            LibItems._removeFromOwner(_from, id, value);
            LibItems._addToOwner(_to, id, value);
    ...
    function transferToParent(
        ...
        require(sender == _from || s.operators[_from][sender], "ItemsTransfer: Not owner and not approved to transfer");
        LibItems._removeFromOwner(_from, _id, _value);
        LibItems._addToParent(_toContract, _toTokenId, _id, _value);
        // LibERC1155Marketplace.updateERC1155Listing(address(this), _id, _from);
    ...
    function batchTransferToParent(
        ...
        for (uint256 i; i < _ids.length; i++) {
            ...
            uint256 value = _values[i];
            LibItems._removeFromOwner(_from, id, value);
            LibItems._addToParent(_toContract, _toTokenId, id, value);
            // LibERC1155Marketplace.updateERC1155Listing(address(this), id, _from);
    ...
    function transferFromTokenApproved(
        ...
        if (_fromContract == address(this)) {
            address owner = s.members[_fromTokenId].userAddress;
            require(
            ...
            require(s.members[_fromTokenId].isActive == true, "ItemsTransfer: Only callable on active Donut");
        } else {
    ...
    function transferFromParent(
        ...
        LibItems._removeFromParent(_fromContract, _fromTokenId, _id, _value);
        LibItems._addToOwner(_to, _id, _value);
    ...
    function batchTransferFromParent(
        ...
        for (uint256 i; i < _ids.length; i++) {
            ...
            LibItems._removeFromParent(_fromContract, _fromTokenId, id, value);
            LibItems._addToOwner(_to, id, value);
    ...
    */
    ...
    ///@notice Used to extract items that have been accidentally burned with donut
    function extractItemsFromBurnedDonut(
        address _to,
        uint256 _tokenId,
        ...
        require(donut.userAddress == address(0), "Donut has not been burned");

        for (uint256 i; i < _itemIds.length; i++) {
            uint256 itemId = _itemIds[i];
            uint256 value = _values[i];
            LibItems._removeFromParent(address(this), _tokenId, itemId, value);
            LibItems._addToOwner(_to, itemId, value);
    ...
    function extractItemsFromDiamond(
        ...
        for (uint256 i; i < _itemIds.length; i++) {
            uint256 itemId = _itemIds[i];
            uint256 value = _values[i];
            LibItems._removeFromOwner(address(this), itemId, value);
            LibItems._addToOwner(_to, itemId, value);
        }
