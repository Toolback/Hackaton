...
library LibWhitelist {
    function _getNewWhitelistId() internal view returns (uint32 whitelistId) {
    ...
    function _getWhitelistFromWhitelistId(uint32 whitelistId) internal view returns (Whitelist storage whitelist) {
        AppStorage storage s = LibAppStorage.diamondStorage();
        require(_whitelistExists(whitelistId), "WhitelistFacet: Whitelist not found");
        whitelist = s.whitelists[whitelistId - 1];
    }

    function _checkWhitelistOwner(uint32 whitelistId) internal view returns (bool isOwner) {
