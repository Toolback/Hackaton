...
import {LibAppStorage, AppStorage, ItemType, DonutProfil, EQUIPPED_WEARABLE_SLOTS} from "./LibAppStorage.sol";
...
library LibItems {
    ...
    function _itemBalancesOfTokenWithTypes(address _tokenContract, uint256 _tokenId)
        internal
        view
    ...
    function _removeFromParent(
        ...
        if (bal == 0) {
            ...
            if (_fromContract == address(this)) {
                _checkWearableIsEquipped(_fromTokenId, _id);
            }
        }
        if (_fromContract == address(this) && bal == 1) {
            DonutProfil storage donut = s.members[_fromTokenId];
            if (
                donut.equippedWearables[LibItems.WEARABLE_SLOT_HAND_LEFT] == _id &&
                donut.equippedWearables[LibItems.WEARABLE_SLOT_HAND_RIGHT] == _id
            ) {
    ...
    function _checkWearableIsEquipped(uint256 _fromTokenId, uint256 _id) internal view {
        AppStorage storage s = LibAppStorage.diamondStorage();
        for (uint256 i; i < EQUIPPED_WEARABLE_SLOTS; i++) {
            require(s.members[_fromTokenId].equippedWearables[i] != _id, "LibItems : Cannot transfer wearable that is equipped");
        }
    }
